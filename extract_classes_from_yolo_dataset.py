...
def main(args):
    ...
    logger.info(f'Extract classes {args.extract_class_nos} from {args.images_list}')
    with open(args.images_list, 'r') as f:
        for image_path in f.readlines():
            ...
            if labels:
                logger.info(f'Found labels on {os.path.basename(image_path)}')
                ...
                logger.info(f'Copy image {image_path} -> {image_dst_path}')
                shutil.copyfile(image_path, image_dst_path)

                logger.info(f'Write labels -> {label_path}')
